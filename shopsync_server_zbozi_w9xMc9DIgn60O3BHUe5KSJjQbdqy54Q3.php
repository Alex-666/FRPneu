...
class OpenSSLEncryption
{
    ...
    public function encode($message)
    {
        ...
        if (openssl_debug == true) {
            echo "SENDIV:" . $iv . "\n";
            echo "SENDMSG:" . $message . "\n";
        }
...
//echo $iv . $ciphertext."\n";
    ...
    public function decode($message)
    {
        ...
        if (substr($message, 0, 3) == "ERR") {
            echo "Chyba prenosu: " . substr($message, 3) . "\n";
            return false;
        } else {
...
function read_batch_query(){
    if ($_POST["method"]==2 && function_exists("openssl_encrypt")) {
        $enc = new OpenSSLEncryption();
        $encmethod = "2";
    } elseif ($_POST["method"]==1 && function_exists("mcrypt_encrypt")) {
        $enc = new MCryptEncryption();
        $encmethod = "1";
    } else {
        die("Není­ k dispozici zadna knihovna pro sifrovani dat.");
    }

$qry=$enc->decode($_POST["data"]);
//echo "\n\n c2:".sha1(cryptcode.$qry)."\n\n";
...
define("ftp_server",'18.198.147.70');
define("ftp_user",'frpneu_synch');
define("ftp_pass",'4i^nqT23');
define("ftp_passive",'True');
define("ftp_mode",'ftp');
define("set_url",'https://www.frpneu.cz');
define("set_sw",'2');
...
define("set_pohoda_db",'Driver={SQL Server Native Client 11.0};Server=WIN-VCA2JUT9F4E\POHODA;Database=StwPh_27458920_2021;Uid=sa;Pwd=stw_19flekc;MARS_Connection=yes');
define("set_apppath",'C:\Program Files (x86)\STORMWARE\POHODA E1\Pohoda.exe');
define("set_agenda",'');
define("set_year",'');
define("filter_ord",'where o.date_added>now()-interval 7 day and  o.order_status_id not in (0,7)');
define("set_bridgepath",'shopsync_server_w9xMc9DIgn60O3BHUe5KSJjQbdqy54Q3.php');
define("set_usebridge",'false');
define("set_bridgebatch",'true');
define("set_fnkey",'w9xMc9DIgn60O3BHUe5KSJjQbdqy54Q3');
define("set_dbfile",'StwPh_27458920_2021');
define("set_pricegroup1",'VO10');
define("set_pricegroup2",'VO10');
define("set_pricegroup3",'');
define("set_shoppergroup1",'5');
define("set_shoppergroup2",'7');
...
define("filter_stock",'Disky;Doplňky;Nákladní;Pneumatiky');
define("lang",'2');
define("taxclass",'11');
...
define("autoscript",'scripts2/xxauto_pohoda_opencart.php');
define("sw_user",'Alex');
define("sw_pass",'2033');
?>
<?php
    ...
    if ($src != null && file_exists($src) && is_file($src)) {
        ...
        while (true) {
            ...
            if (ftp_mode != "sftp") {
                ...
                curl_setopt($ch, CURLOPT_URL, "sftp://" . $ftpserv[0] . "/" . $dest);
...
function getLastUpd($table) {
    ...
    $res2 = qry("select * from shopsync_last where `table`='" . $table . "_reset'");
    if (numrows($res) == 0) {
        qry("insert into shopsync_last set `table`='$table'");
    }
    if (intval(date("H")) < 5 && intval(date("H")) > 1 && (strtotime(@res($res2, 0, "dt")) + 1 * 24 * 60 * 60) < strtotime(@res($res, 0, "dt"))) {
        if (numrows($res2) == 0) {
            qry("insert into shopsync_last set `table`='" . $table . "_reset'");
        }
        qry("update shopsync_last set  dt=now() where `table`='" . $table . "_reset'");
        $GLOBALS["timestart"] = date('Y-m-d H:i:s');
        return "";
    }
...
$map = array(
    ...
    "location" => "location",
    "sklad_tursko" => "count_tursko",
    "sklad_brno" => "count_brno",
    "marze" => "marze",
);
...
$map2_en = array(
    "name" => "name",
    "meta_title" => "name",
    "description" => "desc2_en",
    "meta_description" => "desc1",
);
$map2_sk = array(
    "name" => "name",
    "meta_title" => "name",
    "description" => "desc2_sk",
    "meta_description" => "desc1",
);
...
foreach ($data as $product) {
    if (filter_stock == "" || count(array_diff((explode("/", utf2win($product["storage_name"]))), explode(";", filter_stock))) > 0 || 1 == 1) {
        $prodi++;
        if ($prodi % 100 == 0) {
            echo ("$prodi \r\n");
        }
        ...
        $id = res(qry("select product_id from " . db_prefix . "product where sku='" . $product["code"] . "' and pohoda_id='" . $product["id"] . "'"), 0, "product_id");
        //check change sku
        $old = res(qry("select product_id from " . db_prefix . "product where sku!='" . $product["code"] . "' and pohoda_id='" . $product["id"] . "' and pohoda_id is not null"), 0, "product_id");
        if ($old != null) {
            if ($id != null) {
                qry("update " . db_prefix . "product set status=0, pohoda_id='old_" . $product["id"] . "' where product_id =" . $old);
            } else {
                qry("update " . db_prefix . "product set sku='" . $product["code"] . "' where product_id =" . $old);
            }
        }
        //check dubled sku
        $dubl = res(qry("select product_id from " . db_prefix . "product where status=1 and sku='" . $product["code"] . "' and pohoda_id!='" . $product["id"] . "' and pohoda_id is not null"), 0, "product_id");
        if ($dubl != null) {
            if (!preg_match('/^([0-9]+)$/', $product["id"])) {
                if ($id != null) {
                    qry("update " . db_prefix . "product set status=0 where product_id=" . $id);
                }

                continue;
            } else {
                qry("update " . db_prefix . "product set status=0 where product_id=" . $dubl);
            }
        }
        ...
        $test = qry("select product_id from " . db_prefix . "product where sku='" . $product["code"] . "' and pohoda_id='" . $product["id"] . "' limit 1 ");
        if (numrows($test) == 0) {
            ...
            $id = res(qry("select product_id from " . db_prefix . "product where sku='" . $product["code"] . "' and pohoda_id='" . $product["id"] . "' limit 1"), 0, "product_id");
        ...
        if (isset($product["label"]["clearanceSale"]) and $product["label"]["clearanceSale"] != 0) {
            $sale_cat_id = 1330; //getcat('VĂ˝prodej');
            qry("
            insert into " . db_prefix . "product_to_category
            (product_id, category_id)
            values
        ...
        if ($option["code"] == 'pohoda') {
        $pohoda_count = $option["count"];
        }
        ...
        if ($opt == null) {
        qry("insert into " . db_prefix . "option_value
        (option_id, sort_order)
        values
        ('$option_id', '" . intval($option["code"]) . "')");
        $opt = lastid();
        qry("insert into " . db_prefix . "option_value_description
        ...
        if ($p_opt == null) {
        qry("insert into " . db_prefix . "product_option
        ...
        if ($p_opt_val == null) {
        qry("insert into " . db_prefix . "product_option_value
        ...
        if (strpos($product["id"], "pneub2b") === false && $product["pricegroup"] != 'skip' && count($product["pricegroup"]) > 0) {
            $pgroups = array(99999);
            if ($product["pricegroup"] != null and $pohoda_count > 0) {
                foreach ($product["pricegroup"] as $group => $gprice) {

                    $pgroups[] = $group;
                    $gtest = res(qry("select product_discount_id from " . db_prefix . "product_discount where product_id='$id' and customer_group_id='$group' "), 0, "product_discount_id");

                    if ($gtest != null) {
                        qry("update " . db_prefix . "product_discount
                            set price='" . $gprice . "', quantity=1
                            where product_id='$id' and customer_group_id='$group' ");
                    } else {
                        qry("insert into " . db_prefix . "product_discount (product_id,customer_group_id, priority, quantity, price) values
                            ('$id','$group', 0, 1, '$gprice')");
                    }
                }
            }
        ...
        if (strpos($product["id"], "pneub2b") === false && $product["price_special"] != 'skip') {
            $pgroups = array(99999);
            if ($product["price_special"] != null and $pohoda_count > 0) {
                $group = 1;
                $gprice = $product["price_special"];
                $pgroups[] = $group;
                $gtest = res(qry("select product_special_id from " . db_prefix . "product_special where product_id='$id' and customer_group_id='$group' "), 0, "product_special_id");

                if ($gtest != null) {
                    qry("update " . db_prefix . "product_special
                            set price='" . $gprice . "', date_start='" . $product["price_special_from"] . "', date_end='" . $product["price_special_to"] . "'
                            where product_id='$id' and customer_group_id='$group' ");
                } else {
                    qry("insert into " . db_prefix . "product_special (product_id,customer_group_id, priority, price, date_start, date_end) values
                            ('$id','$group', 0, '$gprice','" . $product["price_special_from"] . "','" . $product["price_special_to"] . "')");
                }
            }
            qry("delete from " . db_prefix . "product_special
                where product_id='$id' and customer_group_id not in ('" . implode("','", $pgroups) . "') ");
        ...
        foreach ($product["params"] as $p) {
            ...
            if ($pid != null) {
                ...
                if (count($p["values"]) == 1) {
                    ...
                    if (preg_match('/^ *[0-9]+,[0-9]+ *$/', $pval)) {$pval = str_replace(',', '.', $pval);}
                ...
                if (numrows($res) > 0 && trim($pval) != "") {
                    foreach ($langs as $lang) {
                        ...
                        if (numrows($test) == 0) {
                            qry("insert into " . db_prefix . "product_attribute set product_id='$id', attribute_id='$pid', language_id='" . $lang . "', text='" . qSafe($pval) . "', value='" . substr(qSafe($pval), 0, 255) . "'");
                        } else {
                            qry("update " . db_prefix . "product_attribute set text='" . qSafe($pval) . "',  value='" . substr(qSafe($pval), 0, 255) . "' where attribute_id='$pid' and language_id='" . $lang . "' and product_id='$id' ");
                        }
                    }
                }
            }
        ...
        if (count($pids) > 0) {
            qry("delete from " . db_prefix . "product_attribute where attribute_id not in ('" . implode("','", $pids) . "') and product_id='$id'");
        }
        //x
        //}
        if ($new == true) {
        ...
        //x
        if ($product["pohoda"] == 1) {
            $sres = qry("select * from shopsync_qty where product_id='$id' limit 1");
            if (numrows($sres) == 0) {
                qry("insert into shopsync_qty set product_id='$id', tursko='" . $product["count_tursko"] . "', brno='" . $product["count_brno"] . "', marze='" . $product["marze"] . "'");
            } else {
                qry("update shopsync_qty set tursko='" . $product["count_tursko"] . "', brno='" . $product["count_brno"] . "', marze='" . $product["marze"] . "' where product_id='$id'");
            }
        }
        ...
        if ($product["sk"] == true) {
            $sres = qry("select * from " . db_prefix . "product_to_store where product_id='$id' and store_id='1' limit 1");
            ...
            qry("delete from " . db_prefix . "product_to_store where product_id='$id' and store_id='1' limit 1");
        }
        ...
        foreach ($langs as $lang) {
            ...
            if (trim($product["desc2"]) == "") {
                $has_desc = false;
                $product["desc2"] = utf2win("KvalitnĂ­ pneumatika za dobrou cenu. DobrĂ© jĂ­zdnĂ© vlastnosti a nĂ­zkĂˇ hluÄŤnost");
            }
            $upd2 = getUpdateCodes($map2, $product);
            if ($lang == 4) {
                $upd2 = getUpdateCodes($map2_en, $product);
            }
            if ($lang == 5) {
                $upd2 = getUpdateCodes($map2_sk, $product);
            }
            if ($lang == 6) {
                $upd2 = getUpdateCodes($map2_hu, $product);
            }

            $upd3 = getUpdateCodes($map3, $product);
            if (numrows($test) == 0) {
                ...
                if ($product["name_change"] != "skip") {
                    if ($has_desc || res($test, 0, "description") == null || res($test, 0, "description") == "" || substr(res($test, 0, "description"), 0, 7) === 'Kvalitn') {
                        ...
                        qry("
                        update " . db_prefix . "product_description set " . $upd3 . " where product_id='$id' and language_id='" . $lang . "'
                        ");
                    }
                }
