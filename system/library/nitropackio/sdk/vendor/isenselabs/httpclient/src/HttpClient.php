...
class HttpClient {
    ...
    public function gethostbyname($host, $isRetry = false) {
        if (empty(HttpClient::$hosts_cache[$host])) {
    ...
    public function connect() {
        ...
        $addrBackup = $this->addr;

        if ($this->isAsync) {
            $this->sock = @stream_socket_client("tcp://$this->addr:$this->port", $errno, $errorMessage, $timeout, STREAM_CLIENT_CONNECT | STREAM_CLIENT_ASYNC_CONNECT, $ctx);
            ...
            $this->sock = @stream_socket_client("tcp://$this->addr:$this->port", $errno, $errorMessage, $timeout, STREAM_CLIENT_CONNECT, $ctx);
        }
