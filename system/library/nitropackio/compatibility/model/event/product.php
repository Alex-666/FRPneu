...
class Product extends NitropackAdminModel {
        ...
        if (null !== $before = $this->getBefore("product", $product_id)) {
            ...
            foreach ($edited_in_stores as $store_id) {
                ...
                if (!$triggered_expected_case) {
                    ...
                    if ($this->hasStateDifference($before, $after, $difference_in, array(), array('date_modified', 'manufacturer_id', 'categories', 'quantity', 'product_image_id', 'product_special_id', 'product_discount_id', 'product_option_id', 'product_option_value_id'))) {
    ...
    private function invalidatePostAddPages($state, $store_id, $reason) {
        // Categories
    ...
    private function isProductActive($product_info) {
        return (bool)$product_info['status'] && $this->checkDatabaseDate($product_info['date_available']);
    }

    private function getProductState($product_id) {
        $state = array();
    ...
    private function getProductName($state) {
        return $state['descriptions'][$this->config->get('config_language_id')]['name'];
    }

    private function customSortBy(&$target, $key, $sort_array_values = true) {
