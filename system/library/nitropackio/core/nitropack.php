...
use NitroPack\NitroPack as SDK;
use NitroPack\HttpClient as Client;
use NitroPack\Filesystem;

class Nitropack {
    const ERROR_LOG_FILENAME = 'nitropackio.error.log';
    const ERROR_LOG_MAX_SIZE = 8388608; // 8 MB
    const DEBUG_LOG_FILENAME = 'nitropackio.debug.log';
    const DEBUG_LOG_MAX_SIZE = 8388608; // 8 MB
    ...
    public function reload() {
        if ($this->setting->has('site_id') && $this->setting->has('site_secret')) {
            ...
            $this->sdk = new SDK($this->setting->get('site_id'), $this->setting->get('site_secret'), null, null, $this->cacheDir());

            if ($this->setting->has('store_id')) {
                $this->sdk->setCachePathSuffix('store_' . $this->setting->get('store_id'));
            }
     ...
     * @param  Callable $success_callback The main execution
     * @param  Callable|null $error_callback   An error handler
     * @return mixed          Return the result of $success_callback but if it fails, return the result of $error, but if it does not exist, return null.
     */
    public static function executionBlock($success_callback, $error_callback = null) {
    ...
    public function serveLocalCache() {
        if ($this->isConnected() && $this->isEnabled() && !headers_sent()) {
    ...
    public function cookie() {
        if (!$this->isInAdmin()) {
            $is_nitropack_disabled = !$this->canCachePage();
    ...
    public function canCachePage() {
        if ($this->hasRegistry()) {
            ...
            return !($is_logged || $has_cart_items || $has_wishlist || ($is_admin_logged && NITROPACK_DISABLED_FOR_ADMIN) || $is_ymm);
        }
    ...
    public function pushTags() {
        if ($this->isConnected() && $this->isEnabled() && $this->isRouteIncluded($this->getRoute()) && !empty($this->tags) && $this->canCachePage()) {
            // Push tags
    ...
    public function validate($site_id, $site_secret) {
        $nitro = new SDK($site_id, $site_secret, null, null, $this->cacheDir());
    ...
    public function tracking(&$output) {
        if ($this->hasRegistry() && !empty($output) && isset($_GET['tracking']) && isset($_SERVER['HTTP_X_NITROPACK_REQUEST'])) {
            ...
            $data['route'] = $this->registryGet('url')->link($route, '', version_compare(VERSION, '2', '>=') ? 'SSL' : true);
    ...
    private function isInAdmin() {
        return basename(DIR_TEMPLATE) == 'template';
    }
