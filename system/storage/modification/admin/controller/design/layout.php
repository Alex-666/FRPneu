...
class ControllerDesignLayout extends Controller {
	...
	protected function getForm() {
        ...
        if (version_compare(VERSION, '2.0.0.0', '>') && version_compare(VERSION, '2.3.0.2', '<')) {
          ...
          $smchup_form_data = (array)$this->model_extension_ocdevwizard_ocdevwizard_setting->getSettingData('smart_checkout_upsell_pro_form_data', (int)$this->config->get('config_store_id'));
          $smchup_text_data = (array)$this->model_extension_ocdevwizard_ocdevwizard_setting->getSettingData('smart_checkout_upsell_pro_text_data', (int)$this->config->get('config_store_id'));
          $smchup_upsell_data = (array)$this->model_extension_ocdevwizard_ocdevwizard_setting->getSettingData('smart_checkout_upsell_pro_upsell_data', (int)$this->config->get('config_store_id'));

          if (isset($smchup_form_data['activate']) && $smchup_form_data['activate'] && $smchup_form_data['activate_static_widget'] && $smchup_upsell_data) {
            $smchup_module_data = array();

            $language_id = $this->model_extension_ocdevwizard_smart_checkout_upsell_pro->getLanguageByCode($this->config->get('config_admin_language'));

            if (isset($smchup_text_data[$language_id]['heading_static_widget'])) {
              $smchup_module_data[] = array(
                'name' => 'Smart Checkout Upsell Pro &gt; '.$smchup_text_data[$language_id]['heading_static_widget'],
                'code' => 'smart_checkout_upsell_pro'
              );
            }

            if ($smchup_module_data) {
              $data['extensions'][] = array(
                'name'   => 'Smart Checkout Upsell Pro',
                'code'   => 'smart_checkout_upsell_pro',
                'module' => $smchup_module_data
              );
            }
          }
        }
        ...
        if (version_compare(VERSION, '2.3.0.2', '>=')) {
          ...
          if (isset($smchup_form_data['activate']) && $smchup_form_data['activate'] && $smchup_form_data['activate_static_widget'] && $smchup_upsell_data) {
            ...
            if (isset($smchup_text_data[$language_id]['heading_static_widget'])) {
              $smchup_module_data[] = array(
                'name' => strip_tags($smchup_text_data[$language_id]['heading_static_widget']),
                'code' => 'smart_checkout_upsell_pro'
              );
            }
        ...
        if (isset($smchup_form_data['activate']) && $smchup_form_data['activate'] && $smchup_form_data['activate_static_widget'] && $smchup_upsell_data) {
          if (version_compare(VERSION, '2.3.0.2', '>=') && version_compare(VERSION, '3.0.0.0', '<')) {
            $data['layout_modules'][] = array(
              'name'       => 'Smart Checkout Upsell Pro',
              'code'       => 'smart_checkout_upsell_pro',
              'edit'       => $this->url->link('extension/ocdevwizard/smart_checkout_upsell_pro', 'token=' . $this->session->data['token'], true),
              'position'   => $smchup_form_data['static_widget_position'],
              'sort_order' => $smchup_form_data['static_widget_sort_order']
            );
          }
          ...
          if (version_compare(VERSION, '3.0.0.0', '>=')) {
            $data['layout_modules'][] = array(
              'name'       => 'Smart Checkout Upsell Pro',
              'code'       => 'smart_checkout_upsell_pro',
              'edit'       => $this->url->link('extension/ocdevwizard/smart_checkout_upsell_pro', 'user_token=' . $this->session->data['user_token'], true),
              'position'   => $smchup_form_data['static_widget_position'],
              'sort_order' => $smchup_form_data['static_widget_sort_order']
            );
          }
