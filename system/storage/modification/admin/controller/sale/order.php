...
class ControllerSaleOrder extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->get['filter_is_paid'])) {
			$url .= '&filter_is_paid=' . $this->request->get['filter_is_paid'];
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_is_paid'])) {
			$filter_is_paid = $this->request->get['filter_is_paid'];
		} else {
			$filter_is_paid = '';
		}
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
                ...
                'shipping_color' => ($result['shipping_code'] == "pickupprague.pickupprague") ? 'color: red;':'',
                'shipping_short_text' => $this->language->get('text_' . $result['shipping_code']),
		...
		$data['sort_is_paid'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=o.is_paid' . $url, true);
		...
		$data['filter_is_paid'] = $filter_is_paid;
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				$data['products'][] = array(
					...
					'option'   		   => $option_data,
					'quantity'		   => $product['quantity'],
                    'sklad'		       => $product_info['location'] == 'pneub2b' ? $product_info['location'] : $product_info['tursko'] . "/" . $product_info['brno'],
			...
			foreach ($custom_fields as $custom_field) {
                ...
                foreach ($customAccountInfo as $key => $info) {
                    ...
                    if ($info['type'] == 'file') {
                        $value = '<a href="index.php?route=extension/module/simple/download&user_token='. $this->session->data['token'] . '&name='.$value.'">'.$value.'</a>';
                    }
            ...
            $data['template_comment_name'] = $this->model_extension_module_fast_comment_status->getComments();
            $data['shipping_address_1'] = $order_info['shipping_address_1'];
            $data['payment_address_1'] = $order_info['payment_address_1'];
            $data['shipping_postcode'] = $order_info['shipping_postcode'];
            $data['payment_postcode'] = $order_info['payment_postcode'];
	...
	public function setPaid() {
		$this->load->model('sale/order');
	  	...
	  	if(isset($this->request->post['order_id']) AND isset($this->request->post['is_paid'])) {

	  		$order_id = (int)$this->request->post['order_id'];
	  		$is_paid = (int)$this->request->post['is_paid'];

	  		if (!$this->user->hasPermission('modify', 'sale/order')) {
				echo json_encode(array("status" => "failed", "is_paid" => "", "order_id" => $order_id));
		  		exit;
			}

	  		$this->model_sale_order->setPaid($order_id, $is_paid);

	  		echo json_encode(array("status" => "ok", "is_paid" => $is_paid, "order_id" => $order_id));
	  		exit;
	  	}
