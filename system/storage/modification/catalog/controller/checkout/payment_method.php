...
class ControllerCheckoutPaymentMethod extends Controller {
	...
	public function save() {
      ...
      // Normal Checkouts
      if($settings['module_gdpr_store_policy_acceptance'] && isset($this->request->post['agree']) && !empty($this->session->data['customer_id']))
    			{
    				$this->load->model('catalog/information');
      ...
      // Guest Checkouts
      if($settings['module_gdpr_store_policy_acceptance'] && isset($this->request->post['agree']) && isset($this->session->data['guest']))
          {
            $this->load->model('catalog/information');
            $this->load->model('extension/module/gdpr');
            $information_info = $this->model_catalog_information->getInformation($this->config->get('config_checkout_id'));
            ...
            $policy_data = array(
              'customer_email' => $this->session->data['guest']['email'] ? $this->session->data['guest']['email'] : '',
              'customer_id' => 0,
              'date_accepted' => date("Y-m-d H:i:s"),
              'policy_id' => $information_info['information_id'],
              'policy_name' => $information_info['title'],
