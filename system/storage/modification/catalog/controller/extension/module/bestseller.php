...
class ControllerExtensionModuleBestSeller extends Controller {
		...
		if ($results) {
			foreach ($results as $result) {
				if ($result['image']) {
                            ...
                            $image = $this->model_tool_image->resize($result['image'], $setting['width'], $setting['height']).'" id="bestseller_'.$result['product_id'].'"';
				} else {
 ...
 'avail_product_quantity'	  => $avail_product_quantity,
			...
			$contents = $this->load->view('extension/module/bestseller', $data);
                            global $aFolder;
                            global $modulesPath;
