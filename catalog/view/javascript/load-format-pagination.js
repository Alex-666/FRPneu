jQuery(document).ready(function($){
    $('#custom-pagination-button').click(function(event) {
        ...
        var active = $(".pagination ul li.active span").text();
        var page = +step+1;
		$.ajax({
			type: "POST",
			url: url,
            ...
            if(response != '' && response != null && response !== "undefined")
            {
				...
				//$(".pagination ul li.active").clone().insertAfter( ".pagination ul li.active" );
				
				
                $(".pagination ul li.active").removeClass("active");
								
                $(".pagination ul li a:contains("+ page +")").parent().addClass("active");   
