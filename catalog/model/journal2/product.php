...
//        }
    ...
    public function getLabels($product_id) {
        ...
        if ($this->journal2->settings->get('label_special_status', 'always') !== 'never') {
            if (self::$sales === null) {
                self::$sales = $this->model_catalog_product->getProducts(array('filter_category_id'=>1330));
            }
            if (!$this->hasLabel($product_id, 'sales') && is_array(self::$sales)) {
                foreach (self::$sales as $product) {
                    if ($product_id == $product['product_id']) {
                        $this->addLabel($product_id, 'stock', $this->language->get('sales'));
                        break;
                    }
                }
            }
