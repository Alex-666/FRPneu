...
class ModelExtensionShippingPrCategoryDPD extends Model {
    function getQuote($address) {
        $this->load->language('extension/shipping/prcategorydpd');
        ...
        if ($tariff) {
            ...
            $quote_data['prcategorydpd'] = array(
            ...
            $method_data = array(
                    ...
                    'quote' => $quote_data,
                    'sort_order' => $this->config->get('shipping_prcategorydpd_sort_order'),
                    'error' => false
            );
        }
                ...
                foreach ($this->cart->getProducts() as $product) {
                    ...
                    if ($product['shipping']) {
                        ...
                        if ($category_tariff) {
                            ...
                            $tariff = array('prcategorydpd_tariff_id' => 'default', 'rate' => $pr_zone['rate'], 'cost' => $pr_zone['cost'], 'cost_basis' => $pr_zone['cost_basis']);
                        }

                        if (isset($tariffs[$tariff['prcategorydpd_tariff_id']])) {
                            $tariffs[$tariff['prcategorydpd_tariff_id']]['quantity'] += $product['quantity'];
                        }
                        else {
                            $tariffs[$tariff['prcategorydpd_tariff_id']] = $tariff;
                            $tariffs[$tariff['prcategorydpd_tariff_id']]['quantity'] = $product['quantity'];
                        }
                    }
                }
    ...
    private function getCategoryTariff($category_id, $pr_zone, $type) {
        ...
        if ($type == 'zone') {
            $table = 'prcategorydpd_zone_category';
            $field = 'prcategorydpd_zone_id';
        }
        else {
            $table = 'prcategorydpd_geozone_category';
            $field = 'prcategorydpd_geozone_id';
        }

        return $this->db->query("SELECT t.* FROM " . $table . " z LEFT JOIN prcategorydpd_tariff t ON z.tariff_id = t.prcategorydpd_tariff_id WHERE " . $field . " = '" . $pr_zone[$field] . "' AND category_id = " . $category_id . " LIMIT 1")->row;
    }
