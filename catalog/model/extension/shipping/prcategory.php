...
class ModelExtensionShippingPrCategory extends Model {
    function getQuote($address) {
        $this->load->language('extension/shipping/prcategory');
        ...
        $query = $this->db->query("SELECT * FROM prcategory_zone WHERE country_id = '" . (int)$address['country_id'] . "' AND zone_id = '" . (int)$address['zone_id'] . "'");
        ...
        if (!$tariff && $geo_zones_ids) {
            ...
            $query = $this->db->query("SELECT * FROM prcategory_geozone WHERE geo_zone_id IN (" . implode(', ', $geo_zones_ids) . ")");
        ...
        if ($tariff) {
            ...
            $quote_data['prcategory'] = array(
                    'code' => 'prcategory.prcategory',
                    'title' => $this->config->get('prcategory_quote_title'),
                    'cost' => $cost,
            ...
            $method_data = array(
                    ...
                    'quote' => $quote_data,
                    'sort_order' => $this->config->get('shipping_prcategory_sort_order'),
                    'error' => false
            );
        }
    ...
    private function getCategoryTariff($category_id, $pr_zone, $type) {
        ...
        if ($type == 'zone') {
            $table = 'prcategory_zone_category';
            $field = 'prcategory_zone_id';
        }
        else {
            $table = 'prcategory_geozone_category';
            $field = 'prcategory_geozone_id';
        }

        return $this->db->query("SELECT t.* FROM " . $table . " z LEFT JOIN prcategory_tariff t ON z.tariff_id = t.prcategory_tariff_id WHERE " . $field . " = '" . $pr_zone[$field] . "' AND category_id = " . $category_id . " LIMIT 1")->row;
    }
