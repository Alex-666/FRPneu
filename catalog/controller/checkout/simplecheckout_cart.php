...
class ControllerCheckoutSimpleCheckoutCart extends SimpleController {
    ...
    public function index() {
        ...
        if (!$this->cart->hasTurskoTwelve()) {
            if ($this->config->get('config_stock_warning')) {
                $this->_templateData['error_warning_tursko_12'] = $this->language->get('error_tursko_12');
            }
            if (!$this->config->get('config_stock_checkout')) {
                $this->_templateData['error_warning_tursko_12'] = $this->language->get('error_tursko_12');
                $this->simplecheckout->addError('cart');
        ...
        //Проверка по складу Tursko
        if (!$this->cart->hasStockTursko()) {
            if ($this->config->get('config_stock_warning')) {
        ...
        foreach ($products as $product) {
            ...
            if ($version >= 200) {
                ...
                //Проверка почтового кода для доставки в этот же день. pickupprague.pickupprague
                $product['postcode_tursko'] = true;
                $postcode = preg_match('#id="payment_address_postcode" value="([^"\']+)#', implode($this->simplecheckout->getRows('payment_address')), $value);
                if ($this->session->data['shipping_method']['code'] == 'pickupprague.pickupprague') {

                    If (substr($value[1], 0, 1) != '1' && !in_array(substr($value[1], 0, 2), array('25', '27'))) {
                        $product['postcode_tursko'] = false;

                    }
                }

                if (!$product['postcode_tursko']) {
                    if ($this->config->get('config_stock_warning')) {
                        $this->_templateData['error_warning_postcode_tursko'] = $this->language->get('error_postcode_tursko');
                    }
                    if (!$this->config->get('config_stock_checkout')) {
                        $this->_templateData['error_warning_postcode_tursko'] = $this->language->get('error_postcode_tursko');
                        $this->simplecheckout->addError('cart');
                ...
                $this->_templateData['products'][] = array(
                    ...
                    'stock_tursko'     => $product['stock_tursko'] ? true : !(!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning')),
                    'postcode_tursko'     => $product['postcode_tursko'] ? true : !(!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning')),
                    'stock_tursko_twelve'     => $product['stock_tursko_twelve'] ? true : !(!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning')),
                    'stock_brno'     => $product['stock_brno'] ? true : !(!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning')),
                    'stock_branch'     => $product['stock_branch'] ? true : !(!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning')),
