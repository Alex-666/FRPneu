...
class ControllerExtensionModuleHeurekaVerified extends Controller {
    ...
    public function verify(&$route, &$args) {
        ...
        if ($order_info && !$order_info['order_status_id'] && $order_status_id) {
            ...
            if ($this->config->get('module_heureka_verified_status')
                && $this->config->get('module_heureka_verified_secret')) {
                require_once(DIR_SYSTEM . 'library/HeurekaOvereno.php');

                if ($order_info['shipping_country'] == 'Česká republika') {
                    $heureka = new HeurekaOvereno($this->config->get('module_heureka_verified_secret'));
                }
            }
            if (isset($heureka)) {
