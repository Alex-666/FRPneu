...
class ControllerExtensionModuleSearchAuto extends Controller {
    ...
    private $tire_params = array('width', 'height', 'diameter', 'season', 'loadindex', 'speedindex', 'manufacturer');
    ...
    public function index() {
        ...
        if ($category_id == $this->config->get('module_search_auto_tire_category')){
            $data['tab'] = 'tire';
            $filters = array();
            if (count($path)){
                if (count($path) == 1){
                ...
                $items = $this->model_catalog_product->getProducts($filters);
                if (count($items)){
    ...
    public function search() {
        ...
        if (in_array($data['tab'],['tire', 'disc'])) {
            ...
            $products = $this->model_extension_module_search_auto->getProducts($filters);
			...
			if ($result['quantity'] <= 0) {
				...
				$stock = $result['quantity'] . ' ks';

			}elseif($result['quantity'] > 20) {
					$stock = 'více než 20 ks';
				}
			}else{
				$stock = $result['quantity']. ' ks (sklad v ČR)';
                                ...
                                if(is_array($autoData[$i][$n][0])) {
                                    ...
                                    if(($n == 0) or ($n < 2 and $countN > 2 )) {
                                        ...
                                        $data['data']['disc'][ $autoData[$i][$n][1] ][] = array(
                                            ...
                                            'r' => $autoData[$i][$n][1],
                                            'e' => $autoData[$i][$n][2],
                                            'n' => "{$autoData[$i][$n][0]} x {$autoData[$i][$n][1]} {$auto['pcd']} ET {$autoData[$i][$n][2]}",
                                            ...
                                            'url' => $this->url->link('extension/module/search_auto/search', "tab=disc&width={$autoData[$i][$n][0]}&diameter={$autoData[$i][$n][1]}&pcd={$auto['pcd']}"),
