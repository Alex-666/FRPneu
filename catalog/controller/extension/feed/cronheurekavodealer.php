...
class ControllerExtensionFeedCronheurekaVODealer extends Controller
{
...
*/
            ...
            if ($product['image'] && trim($product['model']) != '') {
                if ($product["location"] == 'pohoda') {
                    ...
                    if ($product['special']) {
                        ...
                        $output .= '<PRICE_VAT>' . $this->tax->calculate($product['price_dealer'], $product['tax_class_id']) . '</PRICE_VAT>';
                        $output .= '<PRICE>' . $product['price_dealer'] . '</PRICE>';
                    }
        ...
        $file = $basedir . 'zbozi_vo_dealer.xml';
        $fp = fopen($file, "w9");
        fwrite($fp, $output);
        fclose($fp);
