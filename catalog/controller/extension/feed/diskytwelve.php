...
class ControllerExtensionFeedDiskyTwelve extends Controller
{
 ...
 */
    ...
    public function index() {
      ...
      //  $this->build_cat();
        ...
        */
        $sql = "SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.location = 'pohoda' AND p.quantity > 3 AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '0'";
        ...
        foreach ($products_id->rows as $product_id) {
            ...
            If (!in_array($parent, array('220', '373'))) {
                continue;
            }
/*
            ...
            */
            ...
            //if ($product['image'] && trim($product['model']) != '') {
                ...
                */
                ...
                if ($product['special']) {
                    $output .= '<PRICE_NOVAT>' . $this->tax->calculate($product['special'], 0) . '</PRICE_NOVAT>';
                    $output .= '<PRICE_VAT>' . $this->tax->calculate($product['special'], $product['tax_class_id']) . '</PRICE_VAT>';
                } else {
                    $output .= '<PRICE_NOVAT>' . round($this->tax->calculate($product['price'], 0)*0.87) . '</PRICE_NOVAT>';
                    $output .= '<PRICE_VAT>' . round($this->tax->calculate($product['price'], $product['tax_class_id'])*0.87) . '</PRICE_VAT>';
                }


            foreach ($product_attribute_query->rows as $atr) {
                //Šířka ráfku (v palcích)
                if ($atr["attribute_id"] == 3) {
                    $wheel = $atr["text"];
                }
                //Průměr disku (palce)
                if ($atr["attribute_id"] == 1) {
                ...
                //PCD upevňovacích otvorů (mm)
                if ($atr["attribute_id"] == 2) {
                    $pcd = $atr["text"];
                }
                //Průměr centrálního otvoru (mm)
                if ($atr["attribute_id"] == 4) {
                    $dia = $atr["text"];
                }
                //ET
                if ($atr["attribute_id"] == 5) {
                    $et = $atr["text"];
                }
            }
                ...
                $output .= '<ATTRIBUTES>';
                        //Šířka ráfku (v palcích)
                        $output .= '<WHEEL_WIDTH>' . $wheel . '</WHEEL_WIDTH>';
                         //Průměr disku (palce)
        ...
        $file = $basedir . 'DiskyTwelve.xml';
        $filetwo = $basedir . 'noDiskyTwelve.xml';
        ...
        $file3 = $basedir . 'DiskyTwelve_time.xml';
        $fp3 = fopen($file3, "w9");
        fwrite($fp3, $finish);
        fclose($fp3);
        echo "ok";
