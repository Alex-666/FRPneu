...
class ControllerExtensionFeedOcextFeedGeneratorGoogle extends Controller {
	...
	private $eol = "\n";
        ...
        public function getOfferAttrubite($product) {
            ...
            if (isset($template_setting['enable_location_pohoda']) && $template_setting['enable_location_pohoda']) {
            ...
            // Хардкод для сменя ссылки с frpneu.cz на frpneu.sk
            If ($product["setting"]["title"] == "products_pohoda_slovak") {
                $data['link'] = str_replace('frpneu.cz','frpneu.sk',$data['link']);
            }
            ...
            // Хардкод для сменя ссылки с frpneu.cz на frpneu.sk
            If ($product["setting"]["title"] == "products_pohoda_slovak") {
                $data['image_link'] = str_replace('frpneu.cz','frpneu.sk',$data['image_link']);
                $data['additional_image_link'] = str_replace('frpneu.cz','frpneu.sk',$data['additional_image_link']);
            }
        ...
        public function getNameAttributeForType($product,$template_setting,$composite_types,$field_name,$product_field=FALSE) {
            ...
            switch ($composite_types){
                ...
                case 'composite_db_column':
                    ...
                    break;
                case 'special_selection_categories':
                    $result = $this->getSpecialSelectionCategories($this->categories[$product['category_id']]);
                    break;
                default:
        ...
        private function getSpecialSelectionCategories($categories) {

            $parentId = $this->model_catalog_category->getCategory($categories["parentId"]);
            If (in_array($categories["parentId"], array("375", "1284"))){
                If ( $categories["id"] == "1287") {
                    $categories = "Windscreens";
                    return $categories;
                }
                else {
                ...
                If ($parentId["parent_id"] =="156") {
                    $categories = "Wheels";
                    return $categories;

                }
                elseIf ($parentId["parent_id"] == "139" ) {
                    $categories = "Tires";
                    return $categories;
