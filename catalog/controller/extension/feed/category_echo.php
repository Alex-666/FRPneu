...
class ControllerExtensionFeedCategoryEcho extends Controller
{
    ...
    public function index()
    {
        ...
        $categories = $this->model_catalog_category->getCategories("1707");
        //$categories = $this->model_catalog_category->getCategories("1670");
        ...
        foreach ($categories_one as $oldcategory){
            echo  $oldcategory["category_id"] . ', ';
            $categories_2 = $this->model_catalog_category->getCategories($oldcategory["category_id"]);
            foreach ($categories_2 as $result_four) {
                echo  $result_four["category_id"] . ', ';
            }

        }
        $orig_categories = $this->model_catalog_category->getCategories("366");
        //$orig_categories = $this->model_catalog_category->getCategories("156");
        foreach ($orig_categories as $oldcategory){
            echo  $oldcategory["category_id"] . ', ';
            $orig_categories_2 = $this->model_catalog_category->getCategories($oldcategory["category_id"]);
            foreach ($orig_categories_2 as $orig_result_two) {
                echo  $orig_result_two["category_id"] . ', ';
        ...
        foreach ($products_id as $product_id) {
            ...
            //if ($product['image'] != '') {
            $product_categories = $this->model_catalog_product->getCategories($product_id["product_id"]);
