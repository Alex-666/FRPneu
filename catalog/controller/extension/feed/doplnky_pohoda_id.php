...
class ControllerExtensionFeedDoplnkyPohodaId extends Controller
{
            ...
            foreach ($orig_categories as $first_orig) {
                if ($first_category["name"] == $first_orig["name"]) {
                    ...
                    foreach ($categories_2 as $two_category) {
                        ...
                        $categories_3 = $this->model_catalog_category->getCategories($two_category["category_id"]);
                        foreach ($orig_categories_2 as $orig_two) {
                            if ($two_category["name"] == $orig_two["name"]) {
                                ...
                                foreach ($categories_3 as $three_category) {
                                    $orig_categories_3 = $this->model_catalog_category->getCategories($orig_two["category_id"]);
                                    foreach ($orig_categories_3 as $orig_three) {
                                        if ($three_category["name"] == $orig_three["name"]) {
                                            if ($three_category["pohoda_id"]) {
                                                $count++;
                                                $this->db->query("UPDATE " . DB_PREFIX . "category SET pohoda_id = '" . $three_category["pohoda_id"] . "' WHERE category_id = '" . $orig_three["category_id"] . "'");
                                                $this->db->query("UPDATE " . DB_PREFIX . "category SET pohoda_id = null WHERE category_id = '" . $three_category["category_id"] . "'");
                                                $change_cat[] = $orig_three["category_id"];
                                                $clear_cat[] = $three_category["category_id"];
                                            }
