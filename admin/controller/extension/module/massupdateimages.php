<?php
class ControllerExtensionModuleMassUpdateImages extends Controller {
    private $error = array();

    public function index() {

            $this->load->language('extension/module/massupdateimages');

        $this->document->addStyle('view/template/extension/module/massupdateimages.css');
...
if ($this->user->hasPermission('modify', 'extension/module/massupdateimages')) { /// modify permision

if (isset($this->request->post['selected'])) { /// avem produse selectate

if (!isset($this->request->post['product_image']) or !isset($this->request->post['product_description'])) { /// nu avem update
...
//}
    ...
    $image = '';
    if (isset($this->request->post['product_image']) and $this->request->post['product_image']!=''){
        $image = ", image = '". $this->request->post['product_image'] ."'";
    }
    if (isset($this->request->post['product_description']) and $this->request->post['product_description']!=''){
        ...
        if (count($query->rows)>0){
            $this->db->query("UPDATE " . DB_PREFIX . "product_description SET description = '" . nl2br($this->request->post['product_description']). "' WHERE product_id = '" . (int)$product_id . "' and language_id = '" . (int)$this->config->get('config_language_id') . "'");
        }
    }

$this->db->query("UPDATE " . DB_PREFIX . "product p SET date_modified = NOW() " . $image. " WHERE product_id = '" . (int)$product_id . "'");
...
$this->session->data['success'] = $this->language->get('masstxt_succes_products_filtered');
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/massupdateimages', 'user_token=' . $this->session->data['user_token'], true)
        );
        ...
        $data['action'] = $this->url->link('extension/module/massupdateimages', 'user_token=' . $this->session->data['user_token'], true);

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('extension/module/massupdateimages', $data));
    }
