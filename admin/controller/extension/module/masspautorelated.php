...
class ControllerExtensionModulemasspautorelated extends Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        $this->load->language('extension/module/masspautorelated');

        $this->document->addStyle('view/template/extension/module/masspautorelated.css');
        ...
        if (isset($this->request->post['related_category'])) {
            $data['related_categorys'] = $this->request->post['related_category'];
        } else {
            $data['related_categorys'] = array();
        }
        ...
        if (isset($this->request->post['mass_update'])) { /// update button
            ...
            if ($this->user->hasPermission('modify', 'extension/module/masspautorelated')) { /// modify permision

                if (isset($this->request->post['selected'])) { /// avem produse selectate

                    if (!isset($this->request->post['related_category']) AND $this->request->post['upd_mode'] == "ad" AND !isset($this->request->post['product_special']) AND $this->request->post['upd_mode2'] == "ad") { /// nu avem update
                        ...
                        foreach ($this->request->post['selected'] as $product_id) { /// scanare produse
                            ...
                            if ($this->request->post['upd_mode'] == "re") { // remove old discounts
                                $this->db->query("DELETE FROM " . DB_PREFIX . "product WHERE related_category = '" . (int)$product_id . "'");
                            }
                            if (isset($this->request->post['related_category'])) { // add new discounts
                                $related_category = implode(",",$this->request->post['related_category']);


                                $this->db->query("UPDATE " . DB_PREFIX . "product SET related_category = '" . $related_category . "' WHERE product_id = '" . (int)$product_id . "'");
...
" . $plus_join . $plus_where;
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/masspautorelated', 'user_token=' . $this->session->data['user_token'], true)
        );
        ...
        $data['action'] = $this->url->link('extension/module/masspautorelated', 'user_token=' . $this->session->data['user_token'], true);

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('extension/module/masspautorelated', $data));
    }
